#### Server TCP  - secvențial

```python
import socket

# configurarea serverului
server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)  # creeaza un socket TCP
server_socket.bind(('127.0.0.1', 1234))  # leaga socket-ul la IP si port
server_socket.listen(1)  # asculta pentru o conexiune
...
while True:
    ...
    input_data = client_socket.recv(1024).decode()  # primeste date de la client
    print(f"date primite: {input_data}")
    
    response = f"echo: {input_data}"  # pregateste raspunsul
    client_socket.send(response.encode())  # trimite raspunsul inapoi clientului
    
    client_socket.close()  # inchide socket-ul client
...
# functie pentru a gestiona fiecare conexiune de client
...
server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)  # creeaza un socket TCP
server_socket.bind(('127.0.0.1', 1234))  # leaga socket-ul la IP si port
server_socket.listen(5)  # asculta pentru mai multe conexiuni
...
while True:
    ...
    client_thread = threading.Thread(target=handle_client, args=(client_socket,))  # creeaza un fir pentru client
