...
class Consola:
    ...
    #RAPOARTE
    def __report_top_carti1(self):
        """
        Genereaza raport clienti cu carti inchiriate, ordonat dupa nr de carti inchiriate descrescator
        """
        client_list = []
        client_freq = self.__srv_Inch.get_list_of_ids()

        for i, freq in enumerate(client_freq):
            if freq != 0:
                client = self.__srv_Clienti.search_by_id(i)
                client_list.append([client.getNume(), freq])
        client_list = sorted(client_list, key = lambda x: x[1], reverse=True)
        if client_list != []:
            for elem in client_list:
                print("Nume: " + str(elem[0]) + " carti inchiriate " + str(elem[1]))
        else:
    ...
    def __report_top_carti(self):
        ...
        client_freq = self.__srv_Inch.get_list_of_ids()
        for i in range(len(client_freq)):
            if client_freq[i] != 0:
                client = self.__srv_Clienti.search_by_id(i)
                carti_inchiriate = self.__srv_Inch.get_all_for_id(i)  # obține cărțile închiriate pentru un client dat
                for book_id in carti_inchiriate:
                    found = False
    ...
    def __report_client_nr(self):
        """
        Genereaza raportul clienti cu carti inchiriate, ordonat dupa nume
        """
        lista_clienti = []
        clienti_freq = self.__srv_Inch.get_list_of_ids()

        for i in range(len(clienti_freq)):
            if clienti_freq[i] != 0:
                client_l = self.__srv_Clienti.search_by_id(i)
                for client in client_l:
                    lista_clienti.append([client.getNume(), clienti_freq[i]])

        lista_clienti.sort(key=cmp_nume)
        if lista_clienti != []:
    ...
    def __report_carti_nr(self):
        """
        Genereaza raportul clienti cu carti inchiriate, ordonat dupa nr de carti inchiriate crescator
        """
        lista_clienti = []
        clienti_freq = self.__srv_Inch.get_list_of_ids()
        ...
        lista_clienti.sort(key = cmp_carti)
        if lista_clienti != []:
            for elem in lista_clienti:
                print("Nume: " + str(elem[0]) + " carti inchiriate " + str(elem[1]))
        else:
    ...
    def __report_carti_20(self):
        ...
        for i in range(len(client_freq)):
            if client_freq[i] != 0:
                ...
                client_l = self.__srv_Clienti.search_by_id(i)
                for client in client_l:
                    client_list.append([client.getNume(), client_freq[i]])

        client_list = sorted(client_list, key=lambda x: x[1], reverse=True)

        index = len(client_list) * 2 // 10  # Obține 20% din lista clientilor
        client_list = client_list[:index]  # Selectează primele 20%
    ...
    def show_ui(self):
        ...
        while True:
            ...
            if cmd == "rapoarte":
                ...
                print("2. Clienți cu cărți închiriate ordonat dupa nume")
                print("3. Clienți cu cărți închiriate ordonat după numărul de cărți închiriate")
                print('4. Primi 20% dintre cei mai activi clienți (nume client si numărul de cărți închiriate)')
